...
that differs by interface (terminal prompts, TTS, SIP audio) stays in the
caller.
"""
...
class Assistant:
    ...
    async def process_input(self, text: str, source: str = "cli") -> str:
        ...
        # A phone call is a short spoken command, and Groq's fast model caps at
        # 6k tokens/minute — the full 36-tool schema set alone is ~4k, which
        # 413s before the user's words are even counted. When the heuristic
        # cannot narrow a voice turn, send the core set rather than everything.
        if source in {"sip", "voice"} and groups is None:
    ...
    def live_system_prompt(self) -> str:
        """System instructions for an audio-native phone session."""
        return self._build_system_prompt("phone")

    def live_tool_definitions(self) -> list[dict]:
        """Expose tools that can complete without an invisible terminal prompt.

        Phone and browser Live sessions have no terminal confirmation surface.
    ...
    def _build_system_prompt(self, source: str) -> str:
        ...
        voice_note = (
            ...
            if source in {"sip", "voice", "phone"} or source.startswith("phone:")
            else ""
        )
        return (
