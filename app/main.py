...
from app.voice.audio_socket import AudioSocketGateway
from app.voice.gemini_live import LiveQuotaGuard
from app.voice.phone_gateway import PhoneGateway
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="assistant", description="Local-first Mac AI assistant.")
    ...
    parser.add_argument(
        "--phone",
        action="store_true",
        help="Run browser, Plivo, and Twilio Gemini Live WebSockets.",
    )
    parser.add_argument(
        "--sip", action="store_true", help="Run the Asterisk AudioSocket SIP bridge."
    )
    parser.add_argument(
        "--serve", action="store_true", help="Run all live voice gateways."
    )
    parser.add_argument("--phone-host", default="127.0.0.1", help="Phone gateway listen host.")
    parser.add_argument("--phone-port", type=int, default=8080, help="Phone gateway listen port.")
    parser.add_argument("--sip-host", default="127.0.0.1", help="AudioSocket listen host.")
    parser.add_argument("--sip-port", type=int, default=8090, help="AudioSocket listen port.")
    return parser
...
async def _async_main(args: argparse.Namespace) -> int:
    ...
    if args.phone or args.sip or args.serve:
        configure_logging(cfg)
        runtime = await _startup(cfg)
        quota = LiveQuotaGuard(cfg)
        gateways: list[PhoneGateway | AudioSocketGateway] = []
        if args.phone or args.serve:
            gateways.append(
                PhoneGateway(
                    cfg,
                    runtime.assistant,
        ...
        try:
            ...
            labels = []
            if args.phone or args.serve:
                labels.append(f"HTTP/WebSocket {args.phone_host}:{args.phone_port}")
            if args.sip or args.serve:
                labels.append(f"SIP AudioSocket {args.sip_host}:{args.sip_port}")
            print(f"ASTRA live gateway ready: {', '.join(labels)}. Ctrl-C to stop.")
            await asyncio.gather(*(gateway.serve_forever() for gateway in gateways))
